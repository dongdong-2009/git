...
static void s3c2440_nand_enable_hwecc(struct mtd_info *mtd, int mode)
{
	...
	// enable & unlock
	s3c2440nand->NFCONT |= (1<<4);				//初始化ECC编解码器
	s3c2440nand->NFCONT &= ~((1<<5)|(1<<6));	//解锁spare和main区域的ECC
}
...
static int s3c2440_nand_calculate_ecc(struct mtd_info *mtd, const u_char *dat, u_char *ecc_code)
{
	...
	/* lock */
	s3c2440nand->NFCONT |= (1<<5);		//锁定main区域的ECC校验码

	nfmecc0 = s3c2440nand->NFMECC0;
	ecc_code[0] = nfmecc0 & 0xff;		//读取ECC校验码
...
//printf("correct_ECC=0x%02x%02x%02x%02x\n", read_ecc[0], read_ecc[1], read_ecc[2], read_ecc[3]);
	s3c2440nand->NFMECCD0 = (read_ecc[1]<<16) | read_ecc[0];		//讲读取到的ECC值放到该寄存器里面
	s3c2440nand->NFMECCD1 = (read_ecc[3]<<16) | read_ecc[2];		//这样就能自动校验了

	nfestat0 = s3c2440nand->NFESTAT0;								//得到校验的结果
	switch( nfestat0 & 0x3)
	{
	case 0:
		ret = 0;					//无错误
...
//		chip->options      = 0;
		...
		//chip->eccsize      = 512;
		chip->eccsize      = 512;		//对于S3C2440来说，一次硬件ECC可以检验2048个字节，并且生成4个字节的ECC
		chip->eccbytes     = 4;			//因此ecc.size应该为2048，ecc.bytes应该为4
#endif /* CONFIG_TQ2440_NAND_HWECC */
	}

#ifdef CONFIG_TQ2440_NAND_HWECC
	//chip->eccmode       = NAND_ECC_HW8_512;	//每512字节校验1次ECC
	chip->eccmode       = NAND_ECC_HW3_256;		//BYHP 和uboot端保持一致
