...
dm9000_reset(board_info_t * db)
{
	...
	/* RESET device */
	writeb(DM9000_NCR, db->io_addr);        //对相应的寄存器进行操作
	udelay(200);
	writeb(0x03, db->io_data);
	udelay(200);
	
	writeb(DM9000_NCR, db->io_addr);        //为毛这里要操作两遍呢，羞羞哒
	udelay(200);
	writeb(0x03, db->io_data);
	udelay(200);	
}
 ...
 */
static void
dm9000_init_dm9000(struct net_device *dev)              //完成硬件相关操作
{
	board_info_t *db = netdev_priv(dev);
	unsigned int imr;
...
dm9000_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
        ...
        //2.将skb中的数据写入寄存器
	iow(db, DM9000_TXPLL, skb->len);                        //通知要发送的长度
	iow(db, DM9000_TXPLH, skb->len >> 8);

        
        writeb(DM9000_MWCMD, db->io_addr);
        (db->outblk)(db->io_data, skb->data, skb->len);         //把数据包里面的数据写入寄存器

        iow(db, DM9000_TCR, TCR_TXREQ);                         //启动发送
        
        //3.释放skb 
        dev_kfree_skb(skb);                                     //释放skb空间

        //4.在发送中断处理程序中通知协议栈发送数据，在中断中完成，此处不实现
	...
	netif_stop_queue(dev);          //先通知上层停止发送
	db->tx_pkt_cnt++;
	...
	iow(db, DM9000_TCR, TCR_TXREQ);	/* Cleared after TX complete 启动发送 */
...
{
	int tx_status = ior(db, DM9000_TCR);	/* Got TX status 获取发送状态 */
...
dm9000_rx(struct net_device *dev)
{
	...
	do {
		ior(db, DM9000_MRCMDX);	/* Dummy read 执行空读操作 */
		save_mrr = (ior(db, 0xf5) << 8) | ior(db, 0xf4);
		/* Get most updated data */
		rxbyte = ior(db, DM9000_MRCMDX);
		
		if(DM9000_PKT_RDY != rxbyte)            //判断包是否被接收完成
		{
...
//				dev_warn(db->dev, "status check fail: %d\n", rxbyte);
		...
		(db->inblk)(db->io_data, &rxhdr, sizeof(rxhdr));        //读取状态和长度
		...
		if (GoodPacket
		    && ((skb = dev_alloc_skb(RxLen + 4)) != NULL)) {  //加4个字节的头
			skb_reserve(skb, 2);                            //为了保证ip包的4字节对齐
			rdptr = (u8 *) skb_put(skb, RxLen - 4);         //减4表示CFS校验码
...
dm9000_open(struct net_device *dev)
{
	...
	if (request_irq(dev->irq, &dm9000_interrupt, irqflags, dev->name, dev))         //注册中断处理函数
		return -EAGAIN;
...
dm9000_probe(struct platform_device *pdev)
{
	...
	/* Init network device */
	ndev = alloc_etherdev(sizeof(struct board_info));               //分配net_device设备
	if (!ndev) {
		dev_err(&pdev->dev, "could not allocate device.\n");
		return -ENOMEM;
	}

	SET_NETDEV_DEV(ndev, &pdev->dev);       //执行赋值操作
...
//	*((volatile unsigned int *)S3C2410_BWSCON) = (oldval_bwscon & ~(3<<16)) | S3C2410_BWSCON_DW4_16 | S3C2410_BWSCON_WS4 | S3C2410_BWSCON_ST4;
	...
	*((volatile unsigned int *)S3C2410_BANKCON4) = 0x1f7c;
	...
	db = netdev_priv(ndev);         //将ndev的地址加上net_device的长度后，进行32字节的对齐，实际上这个在申请的时候就已经对其了
        printk("Debug by HP: ndev=%x, sizeof(struct net_device)=%x, db=%x\n",\
                                        ndev, sizeof(struct net_device), db);      //Edited by HP

        memset(db, 0, sizeof(*db));     //清0操作

	db->dev = &pdev->dev;
	db->ndev = ndev;

	spin_lock_init(&db->lock);      //自旋锁和互斥锁
	mutex_init(&db->addr_lock);

	INIT_DELAYED_WORK(&db->phy_poll, dm9000_poll_work);             //定义工作队列

	db->addr_res = platform_get_resource(pdev, IORESOURCE_MEM, 0);  //获得相关资源信息，地址寄存器，数据寄存器等
	...
	iosize = res_size(db->addr_res);                                //计算长度
	...
	db->io_addr = ioremap(db->addr_res->start, iosize);             //地址映射
	...
	id_val = ior(db, DM9000_CHIPR);         //凡是io的函数都是读取寄存器，这里读取芯片编号
//	dev_dbg(db->dev, "dm9000 revision 0x%02x  io_mode %02x \n", id_val, db->io_mode);
	...
	ether_setup(ndev);                                      //网卡的相关初始化

	ndev->open		 = &dm9000_open;                //下面的是设置DM9000的操作函数集
	...
	for (i = 0; i < 6; i += 2)
		dm9000_read_eeprom(db, i / 2, ndev->dev_addr+i);        //判断MAC地址是不是有效的
...
static struct platform_driver dm9000_driver = { //这个是平台驱动
	.driver	= {
		.name    = "dm9000",            //平台设备和平台驱动是通过名字来匹配的，这里可以通过平台驱动的名称搜索来找到对应的平台设备
		.owner	 = THIS_MODULE,
	},
	.probe   = dm9000_probe,
