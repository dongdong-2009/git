...
static const struct net_device_ops net_ops = {
	...
	.ndo_start_xmit 	= net_send_packet,              //对应网卡发送函数
...
cs89x0_probe1(struct net_device *dev, int ioaddr, int modular)
{
	...
	writeword(ioaddr, ADD_PORT, PP_ChipID);         //对硬件进行操作
	...
	dev->netdev_ops	= &net_ops;             //初始化操作函数集
...
/* And 2.3.47 had this: */
		...
		for (i = 2; i < CS8920_NO_INTS; i++) {
			if ((1 << i) & lp->irq_map) {
				if (request_irq(i, net_interrupt, 0, dev->name, dev) == 0) {    //数据发送完后，网卡会产生中断 
					dev->irq = i;
...
static int net_send_packet(struct sk_buff *skb, struct net_device *dev)
{
	...
	netif_stop_queue(dev);  //通知上层协议，暂停向网卡发送数据
	...
	/* Write the contents of the packet */
	writewords(dev->base_addr, TX_FRAME_PORT,skb->data,(skb->len+1) >>1);   //将数据写入网卡，发送出去
	spin_unlock_irq(&lp->lock);                                    //数据发送完后，网卡会产生中断                                 
	lp->stats.tx_bytes += skb->len;
	dev->trans_start = jiffies;
	dev_kfree_skb (skb);                    //数据发送走了之后需要将结构释放掉
...
static irqreturn_t net_interrupt(int irq, void *dev_id)
{
	...
	while ((status = readword(dev->base_addr, ISQ_PORT))) {
		...
		handled = 1;
		switch(status & ISQ_EVENT_MASK) {       //网卡有多种类型的中断，这里判断中断的类型
		case ISQ_RECEIVER_EVENT:
			/* Got a packet(s). */
			net_rx(dev);                    //接收到数据时，产生接收中断
			break;
		case ISQ_TRANSMITTER_EVENT:
			lp->stats.tx_packets++;
			netif_wake_queue(dev);	/* Inform upper layers. 发送完成后，通知上层继续发送数据 */
			if ((status & (	TX_OK |
					TX_LOST_CRS |
					TX_SQE_ERROR |
					TX_LATE_COL |
...
{       //接收中断服务函数
	...
	int ioaddr = dev->base_addr;
	status = readword(ioaddr, RX_FRAME_PORT);       //读取接收的状态
	length = readword(ioaddr, RX_FRAME_PORT);       //读接收的长度
	...
	/* Malloc up new buffer. */
	skb = dev_alloc_skb(length + 2);                //分配skb结构，并加上2个字节的包头
	if (skb == NULL) {
	...
	readwords(ioaddr, RX_FRAME_PORT, skb_put(skb, length), length >> 1);    //从寄存器中读取数据，并保存到skb里面去
	if (length & 1)
	...
	if (net_debug > 3) {
        ...
        skb->protocol=eth_type_trans(skb,dev);
	netif_rx(skb);          //将收到的skb协议包，传到上层协议栈
	lp->stats.rx_packets++;
	lp->stats.rx_bytes += length;
}
...
{
	struct net_device *dev = alloc_etherdev(sizeof(struct net_local));    //为net_device结构分配空间
	struct net_local *lp;
	int ret = 0;
