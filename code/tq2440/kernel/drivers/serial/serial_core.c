...
static int uart_startup(struct uart_state *state, int init_hw)
{
	...
	retval = port->ops->startup(port);    //调用uart_ops的startup
	if (retval == 0) {
		if (init_hw) {
			/*
...
static int uart_open(struct tty_struct *tty, struct file *filp)
{
	 ...
	 */
	retval = uart_startup(state, 0);   //调用
...
int uart_register_driver(struct uart_driver *drv)
{
	 ...
	 */
	drv->state = kzalloc(sizeof(struct uart_state) * drv->nr, GFP_KERNEL);   //乘以drv->nr，表示可能有多个设备
	retval = -ENOMEM;    //若没有内存可申请
	if (!drv->state)     //只要申请到内存，drv->state的值就非0  
		goto out;   //内存申请成功后不执行

	normal  = alloc_tty_driver(drv->nr);  //分配drv->nr个tty_driver空间
	if (!normal)
		goto out;
	...
	tty_set_operations(normal, &uart_ops);      //设置tty_operations操作函数
	...
	retval = tty_register_driver(normal);     //注册tty,在这里设置tty的file_operations
 out:
	if (retval < 0) {
